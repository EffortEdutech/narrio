-- Narrio Real Story Seed Pack v1.1 — parser-safe hotfix
-- Fixes v1 issue where raw prose/comment text could be parsed badly by SQL editor copy/paste.
-- Uses base64 JSON payloads so story prose is decoded inside PostgreSQL, not parsed as SQL text.
...
', ''), 'base64'), 'utf8')::jsonb;
  ...
  loop
    ...
    values (
      ...
      b.content_md,
      'import',
      'Real narrative seed v1.1: parser-safe written prose branch for ForkCraft simulation.',
      true,
      v_author_id,
      timezone('utc', now()) - ((112 - b.universe_no) * interval '1 hour')
    )
